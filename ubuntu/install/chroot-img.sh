...
mount_device() {
    if [[ $is_device -eq 0 ]]; then
        ...
        mountpoint -q "${device_name}1" && sudo umount "${device_name}1"
        mountpoint -q "${device_name}2" && sudo umount "${device_name}2"
    fi
    ...
    sudo mkdir -p "$mountpoint/$boot_dir"
    sudo mount "${device_name}1" "$mountpoint/$boot_dir"
