...
mount_efi_partition() {
    ...
    mkdir -vp -m 0600 -- "$arg_rootfs_dir/boot/efi"
...
make_swap_partition() {
    ...
    local _answer="N"
    while [[ "$_answer" != "Y" && "$_answer" != "y" ]]; do
        read -p 'Device for swap partition (e.g. /dev/sda4): ' _swap_dev
        if [[ ! -b "$_swap_dev" ]]; then
...
case "$arg_platform" in
    pc)
        . "$SCRIPT_DIR"/lib/install-btrfs.sh
        function_mount_rootfs=install_btrfs_mount_rootfs
        ;;
    rpi)
        ;;
    *)
